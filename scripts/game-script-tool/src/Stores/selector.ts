...
export const useScenarioSelector = () => {
  ...
  // 시나리오별 이벤트 정렬
  const getSortedEvents = (scenarioCode: string) => {
  ...
  // 이벤트별 선택지 정렬
  const getSortedChoices = (scenarioCode: string, eventId: string) => {
    const scenario = scenarios.find(s => s.scenarioCode === scenarioCode);
    if (!scenario || !scenario.events) return [];
    const event = scenario.events.find(e => e.id.toString() === eventId);
    if (!event || !event.choices) return [];
    return [...event.choices].sort((a, b) => a.scoreWeight - b.scoreWeight);
  };

  // 시나리오 통계
  const getScenarioStats = (scenarioCode: string) => {
    const scenario = scenarios.find(s => s.scenarioCode === scenarioCode);
    ...
    return {
      ...
      totalChoices: scenario.events.reduce(
        (sum, event) => sum + (event.choices?.length || 0),
        0
      ),
      ...
      correctChoices: scenario.events.reduce(
        (sum, event) =>
          sum + (event.choices?.filter(choice => choice.isCorrect).length || 0),
        0
      ),
    };
  };
